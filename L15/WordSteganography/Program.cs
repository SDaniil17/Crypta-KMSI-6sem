...
namespace WordSteganography
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Меню (1) - Выберите действие:");
            ...
            if (selection > 0 && selection < 3)
            {
                ...
                if (selection == 1)
                {
                    Console.WriteLine("Меню (2) - Выберите способ зашифрования:");
                    Console.WriteLine("1) Зашифровать по размеру");
                    Console.WriteLine("2) Зашифровать по цвету");
                ...
                {
                    Console.WriteLine("Меню (3) - Выберите способ расшифрования:");
                    Console.WriteLine("1) Расшифровать по размеру");
                    Console.WriteLine("2) Расшифровать по цвету");
                    int select = int.Parse(Console.ReadLine());
                    if (select == 1)
                    {
                        SizeDecryption();
        ...
        public static void SizeDecryption()
        {
            ...
            Console.WriteLine("Расшифрованное сообщение по размеру: " + BinaryToString(arr));
        }
        ...
        public static void ColorDecryption()
        {
            ...
            Console.WriteLine("Расшифрованное сообщение по цвету: " + BinaryToString(arr));
        }

        public static void SizeEncryption()
        {
            //Initialization 
            Document document = new Document("container_file.docx");
            DocumentBuilder builder = new DocumentBuilder(document);

            double lines_count = document.Sections[0].Body.Paragraphs.Count;
            Console.WriteLine("Вы можете зашифровать только " + Math.Round(lines_count / 8) + " байтов данных в контейнере");
            ...
            {
                Console.WriteLine("Сообщение больше, чем существует места в контейнере");
                return;
            }
        ...
        public static void ColorEncryption()
        {
            ...
            double lines_count = document.Sections[0].Body.Paragraphs.Count;
            Console.WriteLine("Вы можете зашифровать только " + Math.Round(lines_count / 8) + " байтов данных в контенере");
            Console.WriteLine("Введите сообщение для зашифрования:");
            String data = Console.ReadLine();
            String bin = StringToBinary(data);
